...
public class ChatMongoRepository {
	...
	public List<Map> getfreechat(String id, String id2) {
		...
		List<Map> list2 = template.find(
				new Query(Criteria.where("modeId").in(id).andOperator(Criteria.where("modeId").in(id2))), Map.class,
				"freechat");
		return list2;
	...
	public void roomupdate(String id1, String id2, String lasttime, String otherid, String cont) {
		Update u = new Update().set("lastsenddate", (long) (System.currentTimeMillis())).set("lastformat", lasttime)
				.set("sendid", id1).set("cont", cont);
		template.updateMulti(new Query(Criteria.where("modeId").in(id1).andOperator(Criteria.where("modeId").in(id2))),
				u, "freechatroom");
	...
	public void removecount(String id, String id2, String otherid) {
		...
		template.updateMulti(
				new Query(Criteria.where("readid").in(id)
						.andOperator(Criteria.where("otherId").in(id2).andOperator(Criteria.where("id").in(otherid)))),
				u, "freechat");
	}
	...
	public void roomcountupdate(String id, String otherid) { // 채팅방 카운팅
		...
		template.updateMulti(
				new Query(Criteria.where("modeId").in(id).andOperator(Criteria.where("modeId").in(otherid))), u,
				"freechatroom");
	}
