		...
		<div class="row" id="album">
							...
							<div class="col-md-4">
								...
								<div class="card mb-4 shadow-sm">
									...
									<article>
										<div class="thumbImg" style="width: auto; height: 250px;"
											data-target="${i._id }">
											<c:forEach var="p" begin="0" end="0"
												items="${i.file_attach }">
												<a
													href="${pageContext.servletContext.contextPath }/board/board_detail.do?num=${i._id}"
													data-remote="false" data-toggle="modal"
													data-target="#myModal" class="btn btn-default"><img
													class="card-img-top" src="${p }" alt="Card image cap"
													style="width: 328px;"></a>
											</c:forEach>
										</div>
			...
			<script>
				$(".thumbImg").on("mouseover",function() {
									...
									$
											.post(
													"${pageContext.servletContext.contextPath}/indexAjax.do",
													param,
													function(rst) {
														...
														target
																...
																.eq(1)
																.html(
																		"<img src=\"${pageContext.servletContext.contextPath }/img/comment.png\" class=\"icon\"> "
																				+ rst.length);
													});
								});
			</script>
...
<script>
	...
	$(document).ready(function(event) {
		...
		console.log(p+"/"+flag);
			if(flag){
				//ajax
				var scrollHeight = $(window).scrollTop() + $(window).height(); //로딩된전체
				var documentHeight = $(document).height();						//화면에보여지는
				//스크롤이 맨아래로 갔는지 아닌지 확인하는 if문
/* 				console.log("scrolHeight:"+scrollHeight); //현재
					console.log("DocumentHeight:"+documentHeight); //전체 */
					if (scrollHeight +1 > documentHeight && scrollHeight!=documentHeight) { //페이지를 넘어갈때	
